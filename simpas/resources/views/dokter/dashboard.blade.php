    ...
    <div class="container mx-auto mt-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <div class="bg-white p-6 rounded-lg shadow-sm flex items-center">
                ...
                <div class="mx-5">
                    <h4 class="text-gray-500 text-sm font-semibold">Pasien Dalam Antrian</h4>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-4">
                    @forelse($antreanHariIni as $antrian)
                        <div class="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition">
                            ...
                            <a href="{{ route('dokter.periksa.create', $antrian) }}" class="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition">
                                Mulai Periksa
                            </a>
                        </div>
                    @empty
                        <div class="text-center text-gray-500 py-6">Tidak ada antrean pasien untuk saat ini.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="lg:col-span-1 bg-white p-6 rounded-lg shadow-md">
                <h3 class="font-bold text-lg mb-4">Jadwal Praktik Anda</h3>
                @forelse($jadwalPraktik as $jadwal)
                    <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-800 p-4 rounded-r-lg mb-2">
                        <p class="font-bold">{{ $jadwal->hari }}: {{ \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</p>
                        <p class="text-sm">({{ $jadwal->poli->nama_poli }})</p>
                    </div>
                @empty
                    <div class="text-center text-gray-500 py-6">Jadwal praktik belum diatur.</div>
                @endforelse
            </div>
        </div>
    </div>
