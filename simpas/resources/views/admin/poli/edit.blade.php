<x-app-layout>
    <h3 class="text-gray-700 text-3xl font-medium mb-6">Edit Poli</h3>
    <div class="bg-white p-8 rounded-lg shadow-md">
        <form action="{{ route('admin.poli.update', $poli) }}" method="POST">
            @csrf
            @method('PUT')
            <div>
                <label for="nama_poli" class="block text-sm font-medium text-gray-700 mb-1">Nama Poli</label>
                <input type="text" name="nama_poli" id="nama_poli" value="{{ old('nama_poli', $poli->nama_poli) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
